...
@Component({
  selector: 'app-modal-edit-certificate',
  templateUrl: './modal-edit-certificate.component.html',
  styleUrls: ['./modal-edit-certificate.component.scss']
})
export class ModalEditCertificateComponent {
  ...
  ngOnInit():void {
    ...
    this.profileService.getUserProfile(this.userData.jobseekerId).subscribe(
      (response: any) => {
        this.certificate = response.data.jobseekerCertifications;
      },
      (error) => {
      })
  }
